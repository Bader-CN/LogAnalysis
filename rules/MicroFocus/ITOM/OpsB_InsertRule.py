...
class OpsBFiles(ReadFileTemplate):
    ...
    def classifiles(self):
        ...
        if re.findall("administration\.log|administration-audit\.log", self.file, re.IGNORECASE):
            return self.readlog_opsb_itom_di_admin()
        # itom_di_data_access_dpl
        elif re.findall("dataaccess\.log", self.file, re.IGNORECASE):
            return self.readlog_opsb_itom_di_data_access_dpl()

    def readlog_opsb_itom_di_admin(self):
        """
        # administration.log
    ...
    def readlog_opsb_itom_di_data_access_dpl(self):
        """
        # dataaccess.log
        :return: TaskInfo["data"] = SList --> [sqlalchemy obj1, sqlalchemy obj2, ...]
        """
        ...
        if __name__ != "__main__":
            # 针对文件做进一步分析, 来判断加载哪一个 SQL 表
            if re.findall("dataaccess\.log", self.file, re.IGNORECASE):
...
if __name__ == "__main__":
    # 读取测试文件
    file = r"C:\Projects\GitHub\LogAnalysis\test\opsb_log\helm-opsb\helm-opsb__itom-di-data-access-dpl-85b674794d-mwgfw__data-access__vm-opsb.home.local\dataaccess.log"
