...
class OpsBFiles(ReadFileTemplate):
    ...
    def classifiles(self):
        ...
        :return:
        """
        # 针对 itomdipulsar-bookkeeper, 则调用 *** 方法
        if re.findall("itomdipulsar-bookkeeper-\d_.*\.log|itomdipulsar-zookeeper-\d_.*\.log", self.file, re.IGNORECASE):
    ...
    def readlog_opsb_type1(self):
        ...
        if __name__ != "__main__":
            ...
            if re.findall("itomdipulsar-bookkeeper-\d_.*\.log", self.file, re.IGNORECASE):
                from rules.MicroFocus.ITOM.OpsB_SQLTable import ITOM_DI_Pulsar_BookKeeper as OpsBTable
            elif re.findall("itomdipulsar-zookeeper-\d_.*\.log", self.file, re.IGNORECASE):
...
if __name__ == "__main__":
    # 读取测试文件
    file = r"D:\itom-logging-vol\container\itomdipulsar-zookeeper-0_helm-opsb_generate-certificates-a0dab0cc4528e9076f50cc07c97711cadcb7b60e35c13bb6f28ba26d617c6ed4.log.20230120.log"
